...
from config.model_presets import model_presets_manager
...
async def list_models():
    ...
    try:
        models = model_presets_manager.get_available_models()
        model_infos = [
            ModelInfo(
                id=model["id"],
                object=model["object"],
...
async def get_current_model():
    ...
    try:
        current_model = model_presets_manager.get_current_model()
        return CurrentModelResponse(**current_model)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/v1/models/switch")
async def switch_model(request_data: SwitchModelRequest):
    """Switch to a different model."""
    try:
        result = model_presets_manager.switch_model(request_data.model)
...
async def serve_web_interface():
    """Serve the web interface for model switching."""
    try:
        ...
        return HTMLResponse(
            content="<h1>Web Interface Not Found</h1><p>Please ensure the static files are properly installed.</p>",
            status_code=404
        )
